...
Symbol fin_Symbol[100];
void displaySymbolTable(){
	int k=0;
	printf("\nSymbol table (in the format < Index , Name , Return Type , Data Type , Token Type , Size > ): \n\n");
	for(int i=0;i<MAX_SIZE;i++){
		if(st[i] == NULL)
			...
			while(cur){
				fin_Symbol[k]=cur;
				k++;
				//printf("%8d\t %8s\t %8s\t %8d\t \n",cur->index,cur->name, cur->data_type, cur->size);
				cur = cur->next;
			}
		}
	}
	...
	int j;
  	for (int i = 0; i < n ; i++) {  
    	for (j = 0; j < n - i - 1; j++) {  
      		Symbol a1=fin_Symbol[j];
      		Symbol a2=fin_Symbol[j+1];
      		if (a1->index > a2->index) {  
        		Symbol temp = fin_Symbol[j];  
        		fin_Symbol[j] = fin_Symbol[j + 1];  
        		fin_Symbol[j + 1] = temp;  
      		}  
    	} 
    }
    for(int i=0;i<n;i++){
		Symbol cur=fin_Symbol[i];
    	printf("%8d  %8s  %s  %8s  %8s  %8d  \n",cur->index,cur->name,cur->returns, cur->data_type,cur->tokenType, cur->size);
    }   
}
...
void insertSymbolTable(char identifier[], char data_type[]){
	if(searchSymbolTable(identifier, data_type) == -1){
		...
		if(strcmp(data_type, "int") == 0){
			...
			n->tokenType="func";
			n->data_type="-";
			strcpy(n->returns,return_type);
			if(strcmp(n->name,"printf")==0 || strcmp(n->name,"scanf")==0) strcpy(n->returns,"-");
		}
		else{
			n->tokenType="-";
			n->size = 4;
